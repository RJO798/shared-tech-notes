...
## Setup (3 nodes example)
To create a k8s three nodes cluster we need to follow the next steps on each node:  
- **Master**:  
1 install containerd, 2 install kubeadm, 3 Initialize master server, 4 Enable POD Network connectivity  
- **Worker Node 1**:  
1 install containerd, 2 install kubeadm, 5 Join node to the cluster  
- **Worker Node 2**:  
1 install containerd, 2 install kubeadm, 5 Join node to the cluster  
